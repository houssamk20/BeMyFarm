...
async function handleFields(request, env2, pathname) {
  const { DB } = env2;
  ...
  if (pathname === "/v1/fields/stats" && request.method === "GET") {
    ...
    return json([
      ...
      { id: 2, label: "Temperature", value: `${avgTemp.toFixed(0)}\xB0C`, change: "-1\xB0C", trend: "down", icon: "thermometer" },
  ...
  if (pathname === "/v1/fields/yield-forecast" && request.method === "GET") {
    return json([
      { month: "Jan", actual: 4200, forecast: 4e3 },
...
__name(handleFields, "handleFields");

// src/routes/weather.js
var CACHE_TTL = 600;
async function handleWeather(request, env2, pathname) {
  const { CACHE, OPENWEATHER_API_KEY } = env2;
  const url = new URL(request.url);
...
async function handleDisease(request, env2, pathname) {
  const { DB } = env2;
  ...
  if (resolveMatch && request.method === "PATCH") {
    await DB.prepare("UPDATE disease_alerts SET resolved = 1 WHERE id = ?").bind(resolveMatch[1]).run();
    return json({ resolved: true });
  }
  return null;
}
__name(handleDisease, "handleDisease");

// src/routes/water.js
async function handleWater(request, env2, pathname) {
  const { DB } = env2;
  if (pathname === "/v1/water/quality" && request.method === "GET") {
    ...
    const parameters = [
      ...
      { name: "Dissolved Oxygen", value: reading.dissolved_o2, min: 6, max: 12, unit: "mg/L", status: reading.dissolved_o2 >= 6 ? "good" : "warning" },
      { name: "Nitrate", value: reading.nitrate, min: 0, max: 10, unit: "mg/L", status: reading.nitrate <= 10 ? "good" : "warning" },
      { name: "Turbidity", value: reading.turbidity, min: 0, max: 4, unit: "NTU", status: reading.turbidity <= 4 ? "good" : "warning" },
      { name: "Conductivity", value: reading.conductivity, min: 0, max: 600, unit: "\xB5S/cm", status: reading.conductivity <= 600 ? "good" : "warning" },
      { name: "Temperature", value: reading.temp_c, min: 10, max: 25, unit: "\xB0C", status: reading.temp_c >= 10 && reading.temp_c <= 25 ? "good" : "warning" }
    ];
...
// src/routes/ai.js
async function handleAi(request, env2, pathname) {
  const { DB } = env2;
  ...
  if (doneMatch && request.method === "PATCH") {
    await DB.prepare("UPDATE ai_recommendations SET done = 1 WHERE id = ?").bind(doneMatch[1]).run();
    return json({ done: true });
  }
  if (pathname === "/v1/ai/recommendations" && request.method === "POST") {
    const body = await request.json();
    const { field_id, priority, category, title: title2, reason, action, impact, confidence } = body;
    if (!title2 || !priority || !category) return err("title, priority, category required");
    const { meta } = await DB.prepare(`
      INSERT INTO ai_recommendations (field_id,priority,category,title,reason,action,impact,confidence)
      VALUES (?,?,?,?,?,?,?,?)
    `).bind(field_id, priority, category, title2, reason, action, impact, confidence || 0).run();
    return json({ id: meta.last_row_id, created: true }, 201);
  }
...
var src_default = {
  async fetch(request, env2, ctx) {
    ...
    const cors = corsHeaders(env2.CORS_ORIGIN || origin);
    if (request.method === "OPTIONS") {
      return new Response(null, { status: 204, headers: cors });
    }
    let response;
    try {
      if (pathname.startsWith("/v1/fields")) response = await handleFields(request, env2, pathname);
      else if (pathname.startsWith("/v1/weather")) response = await handleWeather(request, env2, pathname);
      else if (pathname.startsWith("/v1/disease")) response = await handleDisease(request, env2, pathname);
      else if (pathname.startsWith("/v1/water")) response = await handleWater(request, env2, pathname);
      else if (pathname.startsWith("/v1/ai")) response = await handleAi(request, env2, pathname);
...
// .wrangler/tmp/bundle-yfm6K0/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-yfm6K0/middleware-loader.entry.ts
